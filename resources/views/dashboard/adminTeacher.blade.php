...
@section('content')
	...
	<section class="flex flex-col md:flex-row justify-between gap-4">
		<div class="card bg-[#4A5B92] text-white w-full md:w-1/3 p-4 md:p-4 lg:p-6 rounded-lg">
			<div class="card-body flex flex-row items-center">
				...
				<div class="ml-auto">
					<p class="text-3xl md:text-4xl lg:text-6xl">{{ $jumlah_guru }}</p>
				</div>
			</div>
		</div>
	</section>
	...
	<!-- Section for Table -->
	<a href="{{ route('add.teacher') }}" class="btn btn-solid-primary mt-6 mb-4 "><i
			class="fa-solid fa-square-plus me-2"></i>Add Teacher</a>
	<section>
					...
					@foreach ($users as $no => $data)
						<tr class="border-b border-gray-200 hover:bg-gray-100">
							...
							<td class="px-4 py-4 flex items-center space-x-4">
								...
								<form action="{{ route('delete.teacher', $data->id) }}" method="post" class="inline">
									@csrf
