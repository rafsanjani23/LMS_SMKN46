				...
				<div class="sm:px-10 px-5">
					...
					<div class="flex sm:flex-row flex-col items-center justify-start gap-5">
						...
						<div class="flex flex-col justify-center sm:text-start text-center">
							<h3 class="sm:text-2xl text-lg font-semibold">{{ $teacher->fullname }}</h3>
							<p class="sm:text-lg text-base">{{ ucfirst($teacher->user->role) }}</p>
						</div>
					</div>
					...
					<div class="flex justify-end mt-5">
						<a class="sm:text-xl text-base" href="/teacher/update-profile/{{ $teacher->nip }}">Change
							Profile</a>
					</div>

					<div class="flex flex-col mt-3 sm:text-xl text-base gap-5">
						<div class="flex flex-col gap-1">
							<h3 class="text-[#4A5B92] font-semibold">NIP</h3>
							<p>{{ $teacher->nip }}</p>
						</div>
						<div class="flex flex-col gap-1">
