		...
		<div class="max-w-5xl w-full bg-white mx-auto rounded-md">
			<div class="pt-7">
				...
				<form action="{{ route('teacher.update.pp', $teacher->nip) }}" method="post" class="px-7"
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-8 text-xl ">
						...
						<div class="flex flex-col justify-center gap-3 relative">
							<label class="pl-3 font-medium text-[#414141] text-opacity-50" for="nip">NIP<span
									class="text-red-500">*</span></label>
							<input id="nip" name="nip" type="text" placeholder="Your NIP" value="{{ $teacher->nip }}"
								class="w-full py-4 px-3 text-[#414141] text-opacity-50 bg-[#e8e8e8] focus:outline-none rounded-md" minlength="18" maxlength="18" required>
							@error('nip')
								<div class="text-red-600 text-xs absolute bottom-[-20px]">
									{{ $message }}
								</div>
						...
						<div class="flex flex-col justify-center gap-3 relative">
							...
							<div class="bg-[#e8e8e8] w-full rounded-md relative overflow-hidden">
								...
								<select name="major" id="major" required
									...
									<option value="" disabled {{ !$teacher->major ? 'selected' : '' }}>Select your major</option>
									<option value="pplg" {{ $teacher->major == 'pplg' ? 'selected' : '' }}>PENGEMBANGAN PERANGKAT LUNAK DAN GIM
									</option>
									<option value="dkv" {{ $teacher->major == 'dkv' ? 'selected' : '' }}>DESAIN KOMUNIKASI VISUAL</option>
									<option value="akl" {{ $teacher->major == 'akl' ? 'selected' : '' }}>AKUNTANSI DAN KEUANGAN LEMBAGA</option>
									<option value="bdp" {{ $teacher->major == 'bdp' ? 'selected' : '' }}>BISNIS DARING DAN PEMASARAN</option>
									<option value="mp" {{ $teacher->major == 'mp' ? 'selected' : '' }}>MANAJEMEN PERKANTORAN</option>
									<option value="umum" {{ $teacher->major == 'umum' ? 'selected' : '' }}>UMUM</option>
								</select>
								@error('major')
									<div class="text-red-600 text-xs absolute bottom-[-20px]">
										{{ $message }}
					...
					<div class="flex flex-col justify-center gap-3 py-4 text-xl relative">
							<label class="px-3 pt-4 font-medium text-[#414141] text-opacity-50" for="profile_picture">Profile Picture</label>
